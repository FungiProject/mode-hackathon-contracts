...
library LibAllowList {
    /// Storage ///
    bytes32 internal constant NAMESPACE = keccak256("com.library.allow.list");
    ...
    /// @dev Adds a token address to the allow list
    /// @param _token the token address to add
    function addAllowedToken(address _token) internal {
        _checkAddress(_token);

        AllowListStorage storage als = _getStorage();

        if (als.tokenAllowed[_token]) return;

        als.tokenAllowed[_token] = true;
        als.tokens.push(_token);
    }

    /// @dev Checks whether a token address has been added to the allow list
    /// @param _token the token address to check
    function tokenIsAllowed(address _token) internal view returns (bool) {
        return _getStorage().tokenAllowed[_token];
    }
